...
// Нормализация имени ингредиента перед отправкой в поиск магазина.
// eda.ru хранит имена в «литературной» форме («Сметана 20%-ная», «Корень имбиря»),
// а ритейлеры индексируют товары короткими словами («Сметана 20%», «Имбирь»).

// Родительный падеж → именительный. Нужен для случаев «Корень имбиря» → «имбирь».
...
const simplifyForSearch = (name) => {
  let s = name.trim();

  // Выкидываем содержимое скобок: «Кинза (кориандр)» → «Кинза»
  s = s.replace(/\s*\([^)]*\)/g, "");

  // «20%-ная» → «20%», «10%-ные» → «10%», «33%-ные» → «33%»
  s = s.replace(
    /(\d+\s*%)[-\s]*(ная|ной|ный|ное|ных|ные|ного|ому|ом|ая|ой|ое|ий|их)/gi,
    "$1"
  );

  // Убираем лидирующие «измерительные» префиксы: «Корень имбиря» → «имбиря»
  s = s.replace(
    /^(Корень|Кусочек|Пучок|Веточка|Ломтик|Лист|Листочки|Кочан|Головка|Зубчик)\s+/i,
    ""
  );

  // Убираем модификаторы-прилагательные, которые мешают ритейлу искать:
  // «Замороженный зелёный горошек» → «зелёный горошек», «Молотый кумин» → «кумин».
  // В цикле, потому что цепочки вроде «Измельчённый свежий тимьян» требуют двух проходов.
  const MODIFIER = /^(Замороженн\S+|Консервированн\S+|Сушен\S+|Сушён\S+|Свеж\S+|Измельч\S+|Молот\S+|Тёрт\S+|Тертый|Очищенн\S+|Жарен\S+|Варен\S+)\s+/i;
  let prev;
  do { prev = s; s = s.replace(MODIFIER, ""); } while (prev !== s);
  // Такие же модификаторы в середине/конце: «Красная консервированная фасоль» → «Красная фасоль»
  s = s.replace(
    /\s+(замороженн\S+|консервированн\S+|сушен\S+|сушён\S+|свеж\S+|измельч\S+|молот\S+|тёрт\S+|тертый|очищенн\S+|жарен\S+|варен\S+)(\s|$)/gi,
    " "
  );

  // Если первое слово — известный родительный падеж, конвертируем в именительный
  const words = s.split(/\s+/).filter(Boolean);
  if (words.length === 1) {
    const lower = words[0].toLowerCase();
    if (GENITIVE_TO_NOMINATIVE[lower]) {
      words[0] = GENITIVE_TO_NOMINATIVE[lower];
    }
  }
...
const getStoreSearchUrl = (ingredientName) =>
  STORES[state.selectedStore].searchUrl(simplifyForSearch(ingredientName));
